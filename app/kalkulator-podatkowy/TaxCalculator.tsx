...
const RYCZALT_OPTIONS: {
  ...
  {
    rate: 2,
    desc: "Sprzedaż produktów rolnych",
    tooltip:
      "Przychody ze sprzedaży wyrobów z własnej działalności rolniczej (przetwory, miód, warzywa itp.)",
  },
  {
    rate: 3,
    desc: "Gastronomia, handel detaliczny",
    tooltip:
      "Działalność gastronomiczna (z wyjątkiem sprzedaży napojów o zawartości alkoholu >1,5%), handel detaliczny, usługi związane z produkcją zwierzęcą",
  },
  {
    rate: 5.5,
    desc: "Produkcja, budownictwo",
    tooltip:
      "Działalność wytwórcza, roboty budowlane, przewóz ładunków pojazdami o ładowności >2 tony, prowizje z handlu",
  },
  {
    rate: 8.5,
    desc: "Usługi inne",
    tooltip:
      "Większość usług nie sklasyfikowanych w wyższych stawkach, np. usługi transportowe, pośrednictwo, wynajem ruchomości, usługi gastronomiczne ze sprzedażą alkoholu",
  },
  {
    rate: -1,
    desc: "Najem (8,5% / 12,5%)",
  ...
  {
    rate: 10,
    desc: "Obrót nieruchomościami",
    tooltip:
      "Kupno i sprzedaż nieruchomości na własny rachunek, przychody z odpłatnego zbycia praw majątkowych lub nieruchomości",
  },
  {
    rate: 12,
    desc: "IT, programowanie",
    tooltip:
      "Usługi związane z oprogramowaniem, doradztwem w zakresie informatyki, przetwarzaniem danych, zarządzaniem stronami internetowymi (hosting)",
  },
  {
    rate: 14,
    desc: "Usługi zdrowotne",
    tooltip:
      "Świadczenie usług w zakresie opieki zdrowotnej (lekarze, dentyści, fizjoterapeuci, pielęgniarki prowadzące prywatną praktykę)",
  },
  {
    rate: 15,
    desc: "Usługi prawne, doradcze, księgowe",
    tooltip:
      "Usługi prawnicze, rachunkowo-księgowe, doradztwo podatkowe, usługi architektoniczne i inżynierskie, reklama, badanie rynku, fotografia",
  },
  {
    rate: 17,
    desc: "Wolne zawody",
    tooltip:
      "Przychody z wolnych zawodów: tłumacze, biegli rewidenci, rzecznicy patentowi, doradcy podatkowi, maklerzy, agenci ubezpieczeniowi (jeśli osobista praca bez zatrudniania)",
  },
];
...
function taxSkala(taxBase: number): number {
  ...
  const t =
    taxBase <= BRACKET_LIMIT
      ...
      : BRACKET_LIMIT * RATE_12 -
        TAX_REDUCTION +
        (taxBase - BRACKET_LIMIT) * RATE_32;
...
function calculate(p: {
  ...
  const carVatPct =
    p.carUsage === "business" ? CAR_VAT_BUSINESS : CAR_VAT_MIXED;
  ...
  if (p.vatMode === "standard") {
    ...
    vatPassThrough = Math.max(
      0,
      Math.round(p.vatOutAnn - p.vatInBizAnn - p.vatInPrivAnn - vatInCar),
    );
  ...
  // ── SKALA ──
  const skalaMonthlyInc = Math.max(
    0,
    p.monthlyRevenue -
      p.businessCosts -
      p.privateCosts -
      Math.round(p.carCosts * carPitPct) -
      zusM,
  );
  const skalaHealthM = healthSkala(skalaMonthlyInc);
  const skalaHealthAnn = Math.round(skalaHealthM * 12);
  const skalaTaxBaseNoIkze = Math.max(
    0,
    Math.round(annRev - totalDeductible - annZus),
  );
  ...
  const skalaTaxBaseNoPriv = Math.max(
    0,
    Math.round(annRev - annBizCosts - annCarDeductible - annZus - ikze),
  );
  ...
  // ── LINIOWY ──
  const linearMonthlyInc = Math.max(
    0,
    p.monthlyRevenue -
      p.businessCosts -
      p.privateCosts -
  ...
  const linearTaxBaseNoIkze = Math.max(
    0,
    Math.round(annRev - totalDeductible - annZus - linearHealthDed),
  );
  ...
  const linearTaxBaseNoPriv = Math.max(
    0,
    Math.round(
      annRev - annBizCosts - annCarDeductible - annZus - linearHealthDed - ikze,
    ),
  );
  ...
  const totalRyczRev = p.ryczaltSources.reduce(
    (s, src) => s + src.amount * 12,
    0,
  );
...
function NumberInput({
  ...
  hint?: string;
  brutto?: boolean;
  onBruttoChange?: (v: boolean) => void;
  vatPct?: number;
}) {
  const showBrutto = brutto !== undefined && onBruttoChange !== undefined;
  const nettoVal =
    showBrutto && brutto && vatPct
      ? Math.round(value / (1 + vatPct / 100))
      : value;
  return (
    <div>
...
function ResultCard({
  ...
  const chartSlices = [
    ...
    ...(r.vatPassThrough > 0
      ? [{ label: "VAT", value: r.vatPassThrough, color: CHART_COLORS[4] }]
      : []),
    ...(r.funduszPracy > 0
      ? [
          {
            label: "Fund. Pracy",
            value: r.funduszPracy,
    ...
    ...(r.ikzeContribution > 0
      ? [{ label: "IKZE", value: r.ikzeContribution, color: CHART_COLORS[7] }]
      : []),
  ];
  ...
  const rows: {
    ...
    {
      label: showVat ? "Przychód brutto" : "Przychód",
    ...
    ...(r.healthDeduction > 0
      ? [
          {
            label: "↳ odliczenie zdrowotnej",
            value: r.healthDeduction,
            dimmed: true as const,
            tip:
              r.label === "Podatek liniowy"
  ...
  return (
    <div
      className={`rounded-2xl border overflow-hidden transition relative ${
        isBest
    ...
    >
      ...
      >
        ...
        <div className="flex items-baseline gap-2">
          <span
            className={`text-2xl font-bold tabular-nums ${isBest ? "text-accent" : "text-gray-900 dark:text-white"}`}
          >
...
export default function TaxCalculator() {
  ...
  const [aiRateHint, setAiRateHint] = useState<{
    id: number;
    text: string;
  } | null>(null);
  ...
  const detectRate = useCallback((sourceId: number, name: string) => {
    ...
    rateDebounce.current = setTimeout(async () => {
      try {
          ...
          prev.map((s) =>
            s.id === sourceId
              ? {
                  ...s,
                  rate: data.rate,
                  isNajem: data.isNajem,
                  ...(data.vatRate != null ? { vatRate: data.vatRate } : {}),
                }
              : s,
          ),
        );
        ...
        setAiLoadingRate(null);
      }
    }, 800);
  }, []);

  const suggestCosts = useCallback(
    async (costType: CostType) => {
      setAiCostLoading(costType);
      try {
        const revDesc = ryczaltSources
          .map((s) => `${s.name || "bez nazwy"}: ${s.amount} zł`)
          .join(", ");
        const costDesc = costItems
          .map((c) => `${c.name || "bez nazwy"} (${c.type}): ${c.amount} zł`)
          .join(", ");
        const res = await fetch("/api/tax-ai", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const monthlyRevenue = ryczaltSources.reduce(
    (s, src) => s + n(src.amount, src.brutto, src.vatRate),
    0,
  );
  ...
  const vatOutAnn =
    vatMode === "standard"
      ? Math.round(
          ryczaltSources.reduce(
            (s, src) =>
              s + n(src.amount, src.brutto, src.vatRate) * (src.vatRate / 100),
            0,
          ) * 12,
        )
      : 0;
    ...
    vatMode === "standard"
      ? Math.round(
          costItems
            ...
            .reduce(
              (s, c) =>
                s + n(c.amount, c.brutto, c.vatRate) * (c.vatRate / 100),
              0,
            ) * 12,
        )
      : 0;
  ...
  const addCost = (type: CostType) => {
    setCostItems((prev) => [
      ...prev,
      { id: nextId, name: "", amount: 0, type, brutto: false, vatRate: 23 },
    ]);
    setNextId((i) => i + 1);
  };
  ...
  const results = useMemo(
    () =>
      calculate({
        ...
        chorobowa,
        prevYearIncome,
        ryczaltSources: ryczaltSources.map((s) => ({
          ...s,
          amount: n(s.amount, s.brutto, s.vatRate),
        })),
        vatMode,
        vatOutAnn,
        vatInBizAnn,
  ...
  const vatExemptExceeded =
    vatMode === "zwolniony" && annualRevenue > VAT_EXEMPT_LIMIT;
  const malyPlusRevExceeded =
    zusStatus === "malyPlus" && annualRevenue > MALY_PLUS_REVENUE_LIMIT;

  const best = [results.skala, results.linear, results.ryczalt].reduce(
    (a, b) => (a.disposable >= b.disposable ? a : b),
  );
      ...
      <div className="grid min-[1800px]:grid-cols-[1fr_1fr] gap-10">
            ...
            <div className="space-y-6 rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900/50 p-6">
              ...
              <div>
                ...
                <div className="space-y-3">
                  {ryczaltSources.map((src) => {
                    const selectedOpt = RYCZALT_OPTIONS.find((o) =>
                      o.isNajem
                        ? src.isNajem
                        : !src.isNajem && o.rate === src.rate,
                    );
                    ...
                    return (
                      ...
                      >
                        ...
                        {aiRateHint?.id === src.id && (
                          <p className="text-[11px] text-accent/80 leading-snug px-0.5">
                            AI: {aiRateHint.text}
                          </p>
                        )}
                        <div
                          ...
                          <div>
                            ...
                            >
                              {RYCZALT_OPTIONS.map((o) => (
                                <option
                                  key={o.isNajem ? "najem" : o.rate}
                                  value={o.isNajem ? "najem" : o.rate}
                                >
                                  {o.isNajem
                ...
                <div className="mt-3 px-1">
                  <div className="flex justify-between items-baseline">
                    <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">
                      Łączny przychód netto:
                    </span>
                    <span className="text-lg font-bold text-accent tabular-nums">
                      {pln(monthlyRevenue)}/mies.
                    </span>
                  </div>
                </div>
              </div>
              ...
              {(["business", "private"] as CostType[]).map((type) => {
                ...
                const label =
                  type === "business"
                    ? "Koszty firmowe"
                    : "Koszty prywatne w firmie";
                const hint =
                  type === "business"
                    ? "Wydatki czysto na firmę"
                    : "Telefon, internet, biuro – i tak ponoszone";
                const total = items.reduce(
                  (s, c) => s + n(c.amount, c.brutto, c.vatRate),
                  0,
                );
                return (
                  <div key={type}>
                          ...
                          >
                            <div className="flex items-center gap-2">
                              <input
                                ...
                                onChange={(e) =>
                                  updateCost(c.id, { name: e.target.value })
                                }
                                className="flex-1 rounded-md border border-gray-200 dark:border-gray-700 bg-transparent px-2 py-1 text-xs text-gray-600 dark:text-gray-400 focus:border-accent focus:ring-1 focus:ring-accent/30 outline-none"
                              />
                              {items.length > 1 && (
                            ...
                            <div
                              className={`grid gap-3 ${vatMode !== "zwolniony" ? "grid-cols-[1fr_auto]" : ""}`}
                            >
                              <div>
                                <div className="flex items-center justify-between mb-1">
              ...
              <div className="border-t border-gray-200 dark:border-gray-700 pt-5 space-y-4">
                ...
                <NumberInput
                  label="Koszty samochodu miesięcznie"
                  value={carCosts}
                  onChange={setCarCosts}
                  step={100}
                  hint="Paliwo, leasing/rata, ubezpieczenie, serwis, myjnia..."
                  brutto={vatMode !== "zwolniony" ? carBrutto : undefined}
                  onBruttoChange={
                    vatMode !== "zwolniony" ? setCarBrutto : undefined
                  }
                  vatPct={carVatRate}
                />
                {vatMode !== "zwolniony" && (
                  <div>
                    ...
                    <select
                      value={String(carVatRate)}
                      onChange={(e) => setCarVatRate(Number(e.target.value))}
                      className="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm focus:border-accent focus:ring-2 focus:ring-accent/30 outline-none"
                    >
                      {VAT_RATES.map((r) => (
                        <option key={r} value={String(r)}>
                ...
                <RadioGroup
                  ...
                  options={[
                    {
                      value: "mixed",
                      label: "Mieszane (firma + prywatnie)",
                      desc: `Odliczenie: ${CAR_PIT_MIXED * 100}% PIT, ${CAR_VAT_MIXED * 100}% VAT`,
                    },
                    {
                      value: "business",
                      label: "Wyłącznie firmowe",
                      desc: `Odliczenie: ${CAR_PIT_BUSINESS * 100}% PIT, ${CAR_VAT_BUSINESS * 100}% VAT (wymaga ewidencji km)`,
                    },
                  ]}
                />
              </div>
            ...
            <div className="space-y-6 rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900/50 p-6">
                ...
                options={[
                  {
                    value: "ulga",
                    label: "Ulga na start",
                    desc: "Pierwsze 6 mies. – brak składek społecznych, tylko zdrowotna",
                  },
                  {
                    value: "maly",
                    label: "Mały ZUS (preferencyjny)",
                    desc: `Mies. 7–30 – podstawa ${pln(SMALL_ZUS_BASE)} → ${pln(Math.round(zusMonthly("maly", true)))}/mies.`,
                  },
                  {
                    value: "malyPlus",
                    label: "Mały ZUS Plus",
                    desc: `Przychód z poprz. roku ≤ ${pln(MALY_PLUS_REVENUE_LIMIT)}. Max ${MALY_PLUS_MAX_MONTHS} mies. w 60. Podstawa od dochodu.`,
                  },
                  {
                    value: "pelny",
                    label: "Pełny ZUS",
                    desc: `Podstawa ${pln(FULL_ZUS_BASE)} → ${pln(Math.round(zusMonthly("pelny", true) + fpMonthly("pelny")))}/mies. (z FP)`,
                  },
                ]}
              />
              ...
              {zusStatus === "malyPlus" && (
                <div className="pl-7 space-y-2">
                  <NumberInput
                    label="Roczny dochód z poprzedniego roku"
                  ...
                  {malyPlusRevExceeded && (
                    <div className="rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 px-3 py-2 text-xs text-red-700 dark:text-red-300">
                      Twój roczny przychód ({pln(annualRevenue)}) przekracza
                      limit Małego ZUS Plus ({pln(MALY_PLUS_REVENUE_LIMIT)}).
              ...
              <div className="border-t border-gray-200 dark:border-gray-700 pt-5 space-y-4">
                  ...
                  options={[
                    {
                      ...
                      desc: `Limit obrotu ${pln(VAT_EXEMPT_LIMIT)}/rok. Brak naliczania i odliczania.`,
                    },
                    {
                      value: "standard",
                      label: "VAT czynny",
                      desc: "VAT naliczony na sprzedaży minus VAT od kosztów",
                    },
                  ]}
                />
                ...
                {vatExemptExceeded && (
                  <div className="rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 px-3 py-2 text-xs text-red-700 dark:text-red-300">
                    Twój roczny przychód ({pln(annualRevenue)}) przekracza limit
                    zwolnienia z VAT ({pln(VAT_EXEMPT_LIMIT)}). Musisz
              ...
              <div className="rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700">
                ...
                <div className="p-4 bg-gray-50 dark:bg-gray-800/50 grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                  ...
                  <span className="tabular-nums font-bold text-right border-t border-gray-200 dark:border-gray-700 pt-1">
                    {pln(Math.round(zusM + fpM + best.healthInsurance / 12))}
                    /mies.
                  </span>
                </div>
              </div>
        ...
        <div className="space-y-10 min-[1800px]:self-start min-[1800px]:sticky min-[1800px]:top-20">
            ...
            <div className="space-y-3">
              {(
                ...
                const maxDisp = Math.max(
                  results.skala.disposable,
                  results.linear.disposable,
                  results.ryczalt.disposable,
                );
                const d = viewMode === "monthly" ? 12 : 1;
                const w =
                  maxDisp > 0 ? (Math.max(0, r.disposable) / maxDisp) * 100 : 0;
                return (
                  <div key={r.label}>
                    <div className="flex justify-between text-sm mb-1">
                      ...
                      <span
                        className={`font-semibold tabular-nums ${r.label === best.label ? "text-accent" : "text-gray-900 dark:text-white"}`}
                      >
                        {pln(Math.round(r.disposable / d))}
                      </span>
                    </div>
                    <div className="h-3 rounded-full bg-gray-100 dark:bg-gray-800 overflow-hidden">
                      <div
                        className={`h-3 rounded-full transition-all duration-500 ${r.label === best.label ? "bg-gradient-to-r from-accent to-indigo-400" : "bg-gray-300 dark:bg-gray-600"}`}
          ...
          <div className="rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 p-4 text-sm text-amber-800 dark:text-amber-200">
            <strong>Uwaga:</strong> Kalkulator ma charakter poglądowy (stawki{" "}
            {YEAR}). Skonsultuj się z księgowym przed podjęciem decyzji.
          </div>
          ...
          <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900/50 overflow-hidden text-sm text-gray-600 dark:text-gray-400">
            ...
            <div className="p-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-2 text-xs">
              ...
              <div>
                <strong>Próg skali:</strong> {pln(BRACKET_LIMIT)}
              </div>
              <div>
                <strong>Stawki skali:</strong> {RATE_12 * 100}% /{" "}
                {RATE_32 * 100}%
              </div>
              <div>
                <strong>Stawka liniowa:</strong> {LINEAR_RATE * 100}%
              </div>
              <div>
                <strong>Zdrowotna (skala):</strong> {HEALTH_RATE_SKALA * 100}%
                dochodu
              </div>
              <div>
                <strong>Zdrowotna (liniowy):</strong> {HEALTH_RATE_LINEAR * 100}
                % dochodu
              </div>
              <div>
                <strong>Pełny ZUS – podstawa:</strong> {pln(FULL_ZUS_BASE)}
              </div>
              <div>
                <strong>Mały ZUS – podstawa:</strong> {pln(SMALL_ZUS_BASE)}
              </div>
              <div>
                <strong>Samochód mieszany:</strong> {CAR_PIT_MIXED * 100}% PIT /{" "}
                {CAR_VAT_MIXED * 100}% VAT
              </div>
              <div>
                <strong>Najem ryczałt:</strong> {NAJEM_RATE_LOW}% do{" "}
                {pln(NAJEM_THRESHOLD)}, {NAJEM_RATE_HIGH}% powyżej
              </div>
              <div>
                <strong>Zwolnienie VAT:</strong> do {pln(VAT_EXEMPT_LIMIT)}{" "}
                obrotu/rok
              </div>
              <div>
                <strong>Mały ZUS Plus:</strong> przychód ≤{" "}
                {pln(MALY_PLUS_REVENUE_LIMIT)}, max {MALY_PLUS_MAX_MONTHS} mies.
              </div>
              <div>
                <strong>IKZE (limit JDG):</strong>{" "}
                {pln(Math.round(IKZE_LIMIT_JDG))}/rok
              </div>
              <div>
                <strong>Min. wynagrodzenie:</strong> {pln(MIN_WAGE)}
              </div>
              <div>
                <strong>Przeciętne wynagr.:</strong> {pln(AVG_SALARY)}
              </div>
            </div>
          </div>
        </div>
